...
#define fileDate 512*512
#define headrerSize 1078
#define fileSize fileDate+headrerSize
...
//-------------------------------UART initialization------------------------------------//
 ...
 status = XAxiDma_SimpleTransfer(&MyDMA, (u32)&fileData[headrerSize],fileSize ,XAXIDMA_DEVICE_TO_DMA);
 if(status!=XST_SUCCESS){
 ...
 status = XAxiDma_SimpleTransfer(&MyDMA, (u32)&fileData[headrerSize],fileSize ,XAXIDMA_DMA_TO_DEVICE);
 if(status!=XST_SUCCESS){
	 print("XAXIDMA_DEVICE_TO_DMA failed");
	 return -1;
 }
